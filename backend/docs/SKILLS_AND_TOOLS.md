...
|----------|------|----------|---------|
| `pricing_quick_quote` | 快速定价建议 | `PricingSkill.calculate_quick_price` | 根据资产ID、权益类型、授权天数计算推荐价格区间 |
| `lineage_summary` | 血缘摘要 | `LineageSkill.get_summary` | 查询数据资产的上游血缘链路 |
...
```python
# SkillRegistry.execute("pricing_quick_quote", {"asset_id": "xxx"})
# 内部流程：
doc = parser.get_document("pricing_quick_quote")
method = get_executor_method(doc.executor, db)  # 动态解析并实例化
result = await method(**arguments)
return {"skill": "pricing_quick_quote", "result": result}
```
