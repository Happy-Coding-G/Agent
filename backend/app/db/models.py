...
agent_task_status_enum = Enum("pending", "running", "completed", "failed", "cancelled", name="agent_task_status")
review_status_enum = Enum("pending", "approved", "rejected", "manual_review", name="review_status")
...
class AgentTasks(Base):
    ...
    agent_type: Mapped[str] = mapped_column(String(32), nullable=False)  # file_query, qa, review, etc.
    status: Mapped[str] = mapped_column(agent_task_status_enum, server_default=text("'pending'"))
    ...
    finished_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(timezone=True))
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"))
    updated_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"), onupdate=datetime.datetime.utcnow)
...
class AssetClusters(Base):
    ...
    graph_cluster_id: Mapped[Optional[str]] = mapped_column(String(128))  # Neo4j cluster ID
    cluster_method: Mapped[Optional[str]] = mapped_column(String(32))  # community_detection, etc.

    asset_count: Mapped[int] = mapped_column(Integer, server_default=text("0"))

    # Publication status
    publication_ready: Mapped[bool] = mapped_column(Boolean, server_default=text("false"))
...
class AssetClusterMembership(Base):
    ...
    cluster_role: Mapped[Optional[str]] = mapped_column(String(32))  # core, peripheral, outlier

    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"))

    cluster: Mapped["AssetClusters"] = relationship("AssetClusters", back_populates="memberships")
...
class ReviewLogs(Base):
    ...
    review_type: Mapped[str] = mapped_column(String(32), nullable=False)  # quality, compliance, completeness
    ...
    # Final status
    final_status: Mapped[str] = mapped_column(review_status_enum, server_default=text("'pending'"))
    reviewer_notes: Mapped[Optional[str]] = mapped_column(Text)

    created_by: Mapped[int] = mapped_column(BigInteger, nullable=False)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"))

    document: Mapped["Documents"] = relationship("Documents", back_populates="review_logs")
...
class TradeListings(Base):
    ...
    # Seller info
    seller_user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    seller_alias: Mapped[str] = mapped_column(String(64), nullable=False)

    # Asset content
    title: Mapped[str] = mapped_column(String(255), nullable=False)
    category: Mapped[str] = mapped_column(String(64), nullable=False, server_default=text("'knowledge_report'"))
    tags: Mapped[List[str]] = mapped_column(postgresql.ARRAY(String(32)), server_default=text("'{}'"))
    ...
    # Status
    status: Mapped[str] = mapped_column(listing_status_enum, server_default=text("'draft'"))
    ...
    # Auto repricing
    auto_reprice_enabled: Mapped[bool] = mapped_column(Boolean, server_default=text("true"))
    last_reprice_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(timezone=True))
    ...
    # Relationships
    seller: Mapped["Users"] = relationship("Users", foreign_keys=[seller_user_id], back_populates="trade_listings")
    orders: Mapped[List["TradeOrders"]] = relationship("TradeOrders", back_populates="listing", foreign_keys="TradeOrders.listing_id")
...
class TradeOrders(Base):
    ...
    buyer_user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    seller_user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    ...
    # Status
    status: Mapped[str] = mapped_column(order_status_enum, server_default=text("'pending'"))

    completed_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(timezone=True))
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"))

    # Relationships
    buyer: Mapped["Users"] = relationship("Users", foreign_keys=[buyer_user_id], back_populates="trade_orders_buyer")
    seller: Mapped["Users"] = relationship("Users", foreign_keys=[seller_user_id], back_populates="trade_orders_seller")
    listing: Mapped["TradeListings"] = relationship("TradeListings", foreign_keys=[listing_id], primaryjoin="TradeOrders.listing_id == TradeListings.public_id", back_populates="orders")
...
class TradeWallets(Base):
    ...
    liquid_credits: Mapped[int] = mapped_column(BigInteger, server_default=text("100000"))  # Default 1000.00
    cumulative_sales_earnings: Mapped[int] = mapped_column(BigInteger, server_default=text("0"))
    cumulative_yield_earnings: Mapped[int] = mapped_column(BigInteger, server_default=text("0"))
    total_spent: Mapped[int] = mapped_column(BigInteger, server_default=text("0"))

    # Yield settings
    auto_yield_enabled: Mapped[bool] = mapped_column(Boolean, server_default=text("true"))
    yield_strategy: Mapped[str] = mapped_column(yield_strategy_enum, server_default=text("'balanced'"))
    last_yield_run_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(timezone=True))
...
class TradeHoldings(Base):
    ...
    # Access control
    access_expires_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(timezone=True))
    download_count: Mapped[int] = mapped_column(Integer, server_default=text("0"))
    last_accessed_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(timezone=True))

    purchased_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"))
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"))
...
class TradeYieldRuns(Base):
    ...
    listing_adjustments: Mapped[List[dict]] = mapped_column(JSONB, server_default=text("'[]'"))
...
class TradeTransactionLog(Base):
    ...
    tx_type: Mapped[str] = mapped_column(String(32), nullable=False)  # deposit, purchase, sale_income, yield_accrual, refund
...
Users.trade_listings = relationship("TradeListings", foreign_keys=[TradeListings.seller_user_id], back_populates="seller")
Users.trade_orders_buyer = relationship("TradeOrders", foreign_keys=[TradeOrders.buyer_user_id], back_populates="buyer")
Users.trade_orders_seller = relationship("TradeOrders", foreign_keys=[TradeOrders.seller_user_id], back_populates="seller")
...
negotiation_status_enum = Enum(
    "pending", "active", "paused", "agreed", "settled", "cancelled", "terminated", "disputed",
    name="negotiation_status"
)
mechanism_type_enum = Enum(
    "fixed_price", "bilateral", "auction", "contract_net", "blackboard",
    name="mechanism_type"
)
message_type_enum = Enum(
    "ANNOUNCE", "BID", "OFFER", "COUNTER", "ACCEPT", "REJECT", "QUERY", "RESPONSE", "COMMIT", "SETTLE",
    name="message_type"
)
...
class NegotiationSessions(Base):
    ...
    # Participants
    seller_user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    buyer_user_id: Mapped[Optional[int]] = mapped_column(BigInteger, ForeignKey("users.id", ondelete="CASCADE"), nullable=True)
    ...
    # Current state
    status: Mapped[str] = mapped_column(negotiation_status_enum, server_default=text("'pending'"))
    current_round: Mapped[int] = mapped_column(Integer, server_default=text("0"))
    current_turn: Mapped[str] = mapped_column(String(16), server_default=text("'seller'"))  # seller, buyer

    # Pricing info (in cents)
    starting_price: Mapped[Optional[int]] = mapped_column(BigInteger)  # For auctions
    reserve_price: Mapped[Optional[int]] = mapped_column(BigInteger)   # Minimum acceptable
    current_price: Mapped[Optional[int]] = mapped_column(BigInteger)   # Current bid/offer
    agreed_price: Mapped[Optional[int]] = mapped_column(BigInteger)    # Final agreed price

    # Blackboard Mode: 底线价格（仅对各自可见，但系统强制执行）
    seller_floor_price: Mapped[Optional[int]] = mapped_column(BigInteger)  # 卖方最低接受价格
    buyer_ceiling_price: Mapped[Optional[int]] = mapped_column(BigInteger)  # 买方最高接受价格

    # Target prices (for LLM decision making)
    seller_target_price: Mapped[Optional[int]] = mapped_column(BigInteger)  # 卖方期望价格
    buyer_target_price: Mapped[Optional[int]] = mapped_column(BigInteger)    # 买方期望价格

    # Winner (for auctions/contract_net)
    winner_user_id: Mapped[Optional[int]] = mapped_column(BigInteger, ForeignKey("users.id"), nullable=True)
    ...
    settlement_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(timezone=True))

    # Timeout handling
    expires_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(timezone=True))
    last_activity_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"))
    ...
    # Relationships
    seller: Mapped["Users"] = relationship("Users", foreign_keys=[seller_user_id], back_populates="seller_negotiations")
    buyer: Mapped[Optional["Users"]] = relationship("Users", foreign_keys=[buyer_user_id], back_populates="buyer_negotiations")
    winner: Mapped[Optional["Users"]] = relationship("Users", foreign_keys=[winner_user_id])
    messages: Mapped[List["AgentMessageQueue"]] = relationship("AgentMessageQueue", back_populates="negotiation", order_by="AgentMessageQueue.created_at")
...
class AgentMessageQueue(Base):
    ...
    # Message routing
    negotiation_id: Mapped[str] = mapped_column(String(32), ForeignKey("negotiation_sessions.negotiation_id", ondelete="CASCADE"), nullable=False)
    from_agent_user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.id"), nullable=False)
    to_agent_user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.id"), nullable=False)
    ...
    # Processing state
    status: Mapped[str] = mapped_column(message_status_enum, server_default=text("'pending'"))
    ...
    # Relationships
    negotiation: Mapped["NegotiationSessions"] = relationship("NegotiationSessions", back_populates="messages")
    from_user: Mapped["Users"] = relationship("Users", foreign_keys=[from_agent_user_id], back_populates="sent_messages")
    to_user: Mapped["Users"] = relationship("Users", foreign_keys=[to_agent_user_id], back_populates="received_messages")
...
class NegotiationHistorySummary(Base):
    ...
    # 摘要层级
    layer: Mapped[int] = mapped_column(Integer, server_default=text("1"))  # 1=原始, 2=第一层摘要, 3=第二层摘要
    ...
    price_trajectory: Mapped[dict] = mapped_column(JSONB)  # 价格轨迹: {"start": 120, "end": 85, "trend": "converging"}
    key_events: Mapped[list] = mapped_column(JSONB)  # 关键事件: ["buyer_reduced_20", "seller_reduced_5", ...]
    sentiment_summary: Mapped[Optional[dict]] = mapped_column(JSONB)  # 情绪摘要: {"buyer_sentiment": "positive", "seller_sentiment": "neutral"}
...
class UserAgentConfig(Base):
    ...
    pricing_strategy: Mapped[str] = mapped_column(String(32), server_default=text("'negotiable'"))  # fixed, negotiable, aggressive
    negotiation_style: Mapped[str] = mapped_column(String(32), server_default=text("'balanced'"))  # conservative, balanced, aggressive

    # Auto-negotiation settings
    auto_accept_threshold: Mapped[Optional[float]] = mapped_column(Float)  # Auto-accept if offer >= this % of target
    auto_counter_threshold: Mapped[Optional[float]] = mapped_column(Float)  # Auto-counter if offer >= this % of reserve
    max_auto_rounds: Mapped[int] = mapped_column(Integer, server_default=text("5"))

    # Notification settings
    notify_on_new_bid: Mapped[bool] = mapped_column(Boolean, server_default=text("true"))
    notify_on_offer: Mapped[bool] = mapped_column(Boolean, server_default=text("true"))
    notify_on_agreement: Mapped[bool] = mapped_column(Boolean, server_default=text("true"))
...
class ConversationSessions(Base):
    ...
    status: Mapped[str] = mapped_column(String(16), server_default=text("'active'"))  # active/archived/deleted
    ...
    # 时间
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"))
    last_message_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"))
    ended_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(timezone=True))

    # 关系
    messages: Mapped[List["ConversationMessages"]] = relationship("ConversationMessages", back_populates="session", order_by="ConversationMessages.created_at")
...
class ConversationMessages(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    message_id: Mapped[str] = mapped_column(String(32), unique=True, nullable=False)
    session_id: Mapped[str] = mapped_column(String(32), ForeignKey("conversation_sessions.session_id", ondelete="CASCADE"), nullable=False)
    user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    ...
    # 关联 Agent 类型
    agent_type: Mapped[Optional[str]] = mapped_column(String(32))  # QA/DataProcess/Trade/...
    ...
    # 关系
    session: Mapped["ConversationSessions"] = relationship("ConversationSessions", back_populates="messages")
...
class UserPreferences(Base):
    ...
    # 偏好类型
    pref_type: Mapped[str] = mapped_column(String(32), nullable=False)  # search/response/style/price/...
    ...
    source: Mapped[str] = mapped_column(String(16), server_default=text("'implicit'"))  # explicit/implicit/inferred
...
class UserMemories(Base):
    ...
    # 类型
    memory_type: Mapped[str] = mapped_column(String(32), nullable=False)  # fact/preference/goal/relationship/...

    # 向量嵌入（用于语义检索）
    embedding: Mapped[Optional[object]] = mapped_column(Vector(1536), nullable=True)

    # 来源
    source: Mapped[str] = mapped_column(String(32), server_default=text("'conversation'"))  # conversation/document/manual
...
class AgentDecisionLogs(Base):
    ...
    agent_type: Mapped[str] = mapped_column(String(32), nullable=False)  # QA/DataProcess/Trade/...

    # 决策信息
    decision: Mapped[str] = mapped_column(String(64), nullable=False)  # action taken
    context: Mapped[dict] = mapped_column(JSONB, server_default=text("'{}'"))  # 决策时的上下文
...
class AgentIntermediateResults(Base):
    ...
    # 结果数据
    result_type: Mapped[str] = mapped_column(String(32), nullable=False)  # text/json/file/...
...
# Add back-populates to Users
Users.seller_negotiations = relationship("NegotiationSessions", foreign_keys=[NegotiationSessions.seller_user_id], back_populates="seller")
Users.buyer_negotiations = relationship("NegotiationSessions", foreign_keys=[NegotiationSessions.buyer_user_id], back_populates="buyer")
Users.sent_messages = relationship("AgentMessageQueue", foreign_keys=[AgentMessageQueue.from_agent_user_id], back_populates="from_user")
Users.received_messages = relationship("AgentMessageQueue", foreign_keys=[AgentMessageQueue.to_agent_user_id], back_populates="to_user")
Users.agent_configs = relationship("UserAgentConfig", back_populates="user")

# Memory management relationships
ConversationSessions.user = relationship("Users", back_populates="conversation_sessions")
...
class SpaceMembers(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    space_id: Mapped[str] = mapped_column(String(32), ForeignKey("spaces.public_id", ondelete="CASCADE"), nullable=False)
    user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    ...
    invite_status: Mapped[str] = mapped_column(String(16), server_default=text("'active'"))  # pending/active/removed

    # 统计
    joined_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"))
    last_accessed_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(timezone=True))

    # 通知偏好
    notification_preferences: Mapped[dict] = mapped_column(JSONB, server_default=text("'{}'"))
...
class ResourceACL(Base):
    ...
    # 资源信息
    resource_type: Mapped[str] = mapped_column(String(32), nullable=False)  # space/file/asset/knowledge
    ...
    can_execute: Mapped[bool] = mapped_column(Boolean, server_default=text("false"))  # 用于 Agent 执行

    # 权限条件 (ABAC)
    conditions: Mapped[Optional[dict]] = mapped_column(JSONB)  # {"time_range": "9-18", "ip_whitelist": [...]}

    # 继承
    inherit_from: Mapped[Optional[str]] = mapped_column(String(32))  # 父资源ID
    priority: Mapped[int] = mapped_column(Integer, server_default=text("0"))  # 权限优先级 (高覆盖低)
...
class AuditLogs(Base):
    ...
    user_email: Mapped[Optional[str]] = mapped_column(String(255))  # 快照，即使用户删除也能追溯
    ...
    # 操作信息
    action: Mapped[str] = mapped_column(String(64), nullable=False)  # user.login, file.download, etc.
    ...
    # 结果
    result: Mapped[str] = mapped_column(String(16), server_default=text("'success'"))  # success/failure/denied/error
    error_message: Mapped[Optional[str]] = mapped_column(Text)

    # 风险评分 (0-1)
    risk_score: Mapped[float] = mapped_column(Float, server_default=text("0.0"))
    risk_reasons: Mapped[Optional[list]] = mapped_column(JSONB)  # ["unusual_ip", "off_hours_access"]

    # 处理状态
    alert_sent: Mapped[bool] = mapped_column(Boolean, server_default=text("false"))
    reviewed_by: Mapped[Optional[int]] = mapped_column(BigInteger, ForeignKey("users.id"))
...
class DataLineage(Base):
    ...
    # 源信息
    source_type: Mapped[str] = mapped_column(String(32), nullable=False)  # upload/api/agent_generation/import
    ...
    # 转换链
    transformations: Mapped[list] = mapped_column(JSONB, server_default=text("'[]'"))  # [{"step": "chunking", "timestamp": "...", "config": {...}}]

    # 当前状态
    current_entity_type: Mapped[str] = mapped_column(String(32), nullable=False)  # file/chunk/knowledge/asset
    current_entity_id: Mapped[str] = mapped_column(String(32), nullable=False)

    # 血缘关系 (树形结构)
    parent_lineage_id: Mapped[Optional[str]] = mapped_column(String(32), ForeignKey("data_lineage.lineage_id"))
    ...
    # 关系
    parent: Mapped[Optional["DataLineage"]] = relationship("DataLineage", remote_side=[lineage_id], back_populates="children")
    children: Mapped[List["DataLineage"]] = relationship("DataLineage", back_populates="parent")
...
class CollaborationOperations(Base):
    ...
    # 操作类型
    operation_type: Mapped[str] = mapped_column(String(32), nullable=False)  # edit/delete/create/move
    entity_type: Mapped[str] = mapped_column(String(32), nullable=False)  # file/markdown/graph_node
    ...
    operation_summary: Mapped[Optional[str]] = mapped_column(String(255))  # 人类可读摘要

    # 向量时钟 (用于冲突解决)
    vector_clock: Mapped[dict] = mapped_column(JSONB, nullable=False)  # {"user_123": 5, "user_456": 3}

    # 时间戳
    operation_timestamp: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"))

    # 同步状态
    synced_to_clients: Mapped[list] = mapped_column(JSONB, server_default=text("'[]'"))  # ["client_id_1", "client_id_2"]


# Add back-populates for new models
Users.space_memberships = relationship("SpaceMembers", foreign_keys=[SpaceMembers.user_id], back_populates="user")
Users.invited_members = relationship("SpaceMembers", foreign_keys=[SpaceMembers.invited_by], back_populates="inviter")
Users.audit_logs = relationship("AuditLogs", foreign_keys=[AuditLogs.user_id], back_populates="user")
...
SpaceMembers.user = relationship("Users", foreign_keys=[SpaceMembers.user_id], back_populates="space_memberships")
SpaceMembers.inviter = relationship("Users", foreign_keys=[SpaceMembers.invited_by], back_populates="invited_members")
...
AuditLogs.user = relationship("Users", foreign_keys=[AuditLogs.user_id], back_populates="audit_logs")
...
CollaborationOperations.space = relationship("Spaces", back_populates="collaboration_ops")

Spaces.collaboration_ops = relationship("CollaborationOperations", back_populates="space")
