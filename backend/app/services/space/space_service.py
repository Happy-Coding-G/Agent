...
class SpaceService(SpaceAwareService):
    """空间服务 - 继承 SpaceAwareService"""

    def __init__(self, db: AsyncSession):
        super().__init__(db)

    async def list_spaces(
        self, current_user: Users, limit: int, offset: int
    ) -> Sequence[Spaces]:
    ...
    async def delete_space(self, current_user: Users, space_public_id: str) -> None:
        try:
            async with self.db.begin():
                target = await self.spaces.get_by_public_id_for_owner(
                    space_public_id, current_user.id
                )
                if not target:
            ...
            raise ServiceError(
                500, "Delete failed. Please ensure the space is empty before deleting."
            )
