"""QA Tools - Hybrid vector+graph search and answer generation.

Extracted from the legacy QAAgent LangGraph implementation.
"""
...
def build_tools(registry: "AgentToolRegistry") -> List[StructuredTool]:
    ...
    async def qa_hybrid_search(query: str, space_id: str, top_k: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            if not space:
                return {"success": False, "error": "Space not found", "results": []}
            ...
            try:
                ...
                vector_results = strong or vector_results[:max(top_k * 2, top_k)]
                        ...
                        if driver:
                            ...
                            with driver.session(database=settings.NEO4J_DATABASE) as session:
                                cypher = """
                                    ...
                                    LIMIT $limit
                                """
                                records = session.run(cypher, graph_ids=graph_ids, terms=terms, limit=graph_recall)
                            ...
                            if results:
                                ...
                                graph_results = results[:graph_recall]
            except Exception as e:
    ...
    return [
        StructuredTool.from_function(
            ...
            description="执行向量+知识图谱混合检索，召回与用户查询相关的文档片段。返回带有评分和来源的上下文列表。",
        ...
        StructuredTool.from_function(
            ...
            description="基于检索到的上下文片段生成自然语言回答。必须传入 contexts（来自 qa_hybrid_search 的结果）。",
