...
class SkillLoader:
    ...
    目录结构:
        skills/packages/skill-name/Skill.md
        skills/packages/skill-name/examples/*.md
        skills/packages/skill-name/references/*.md
    """
        ...
        if skill.skills:
            ...
            for sub_name in skill.skills:
                if sub_name == skill_name:
                    continue  # 跳过自引用
                sub_skill = self.load_level1(sub_name)
                if sub_skill and sub_skill.capability_type not in ("skill", "prompt"):
                    continue  # 跳过 agent/subagent/tool 引用，避免注入完整 agent body
    ...
    def list_skills(self) -> List[str]:
        ...
        # 2) Configured markdown definition directories
        for docs_dir in self.docs_dirs:
